#!/usr/bin/env node
/**
 * Extract healing-only SLICE (subset of agentic-platform) → dist-packages/agentic-healing-platform.
 * For full monorepo use extract-agentic-platform-repo.mjs instead.
 */
...
const DOCS = [
  ...
  'CTO-AI-Director-Agentic-Healing-Brief.md',
  'UAT-Test-Plan-and-Usage.md',
...
const SCRIPTS = [
  'extract-agentic-platform-repo.mjs',
  'publish-agentic-platform-github.sh',
...
function writeRootPackageJson() {
  return {
    ...
    scripts: {
      ...
      'build:agent': 'npm run build:core && npm run build -w ai-healing-agent',
      'build:cypress': 'npm run build:agent && npm run build -w ai-healing-cypress',
      'build:selenium': 'npm run build:agent && npm run build -w ai-healing-selenium',
...
| \`packages/ai-healing-agent\` | Shared local agent loop (observe → tools → act → reflect) |
